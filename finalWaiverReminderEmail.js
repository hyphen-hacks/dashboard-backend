...
peopleRef.get().then(data => {
  data.forEach(snap => {
    ...
    if (cleanedPerson.accepted) {
      if (!cleanedPerson.waiver_completed) {
        ...
        sendEmail(cleanedPerson.email, 'd-0b11c43baa2740d1b2dbb591172e71c9', cleanedPerson.name, {
  ...
  fs.writeFile('./private/finalEmailWaivers.json', JSON.stringify(allPeople), (e) => {
    console.log(e)
